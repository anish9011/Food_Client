...
export default function PublicOrderDetails() {
  ...
  const itemId = searchParams.get("itemId"); // Get itemId from query string
  ...
  const fetchCartItems = async () => {
    try {
      ...
      console.log('Fetching cart for userId:', userId); // Log the userId for debugging

      // If an itemId exists, fetch the cart items associated with that itemId
      const url = itemId 
        ? `http://localhost:5000/api/usercart/${itemId}` // For a specific itemId
        : 'http://localhost:5000/api/usercart'; // Otherwise, fetch the entire cart
  ...
  useEffect(() => {
    fetchCartItems();
  }, [itemId]); // Re-run the request if itemId changes

  // Construct the orderPageUrl with userId
  const orderPageUrl = () => {
    const token = localStorage.getItem('token'); // Retrieve the token from localStorage
    if (token) {
      const decodedToken = jwtDecode(token); // Decode the token to get userId
      const userId = decodedToken.userId; // Extract userId
      return `${window.location.origin}/publicorderpage?userId=${userId}`; // Return the URL with userId
    }
    return '#'; // Return a fallback URL if no token exists
  };
